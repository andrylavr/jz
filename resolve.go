...
func (r *RequireModule) resolve(modpath string) (module *js.Object, err error) {
	...
	var start string
	err = nil
	isHTTP := strings.HasPrefix(modpath, "http:") || strings.HasPrefix(modpath, "https:")
	if isHTTP {
		start = ""
	} else if path.IsAbs(origPath) {
		start = "/"
	} else {
	...
	if isHTTP {
		module, err = r.loadAsFileOrDirectory(modpath)
		if err == nil && module != nil {
			r.modules[modpath] = module
		}
...
func (r *RequireModule) checkImport(name string) string {
	if importURL, ok := r.runtime.ImportMap[name]; ok {
		return importURL
	}
	for importName, importURL := range r.runtime.ImportMap {
		...
		return strings.Replace(name, importName, importURL, 1)
	}
	return name
}
